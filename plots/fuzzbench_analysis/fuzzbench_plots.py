...
def read_benchmarks(home_path, benchmark_dir_names):
    ...
    for b in benchmark_dir_names:
        try:
            data = pd.read_csv(home_path + '/' + b + '/data.csv.gz')
...
def load_benchmarks():
    home_mat = SCRIPT_DIR + '/../' + '/wmatt/report-data'
    benchmarks_matt = [
    ...
    home_bean = SCRIPT_DIR + '/..' + '/bean/benchmarks/report-data'
    benchmarks_bean = [
...
def fuzzbench_analysis(experiment_data = load_benchmarks()):
    ...
    # print(results.rank_by_unique_coverage_average_normalized_score.to_frame())
...
if __name__ == '__main__':
    ...
    fuzzbench_analysis(experiment_data)
