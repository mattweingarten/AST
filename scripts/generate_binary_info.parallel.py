...
def handle_folder(curr_dir, flags, benchmarks, dir):
    ...
    if (len(elfs) > 0):
        ...
        for elf in elfs:
            ...
            command = [
                "python3",
                "./scripts/handle_single_binary.parallel.py",
                elf,
                get_flag(elf, flags),
    ...
    for folder in folders:
        handle_folder(folder, flags, benchmarks, dir)
...
dir = SCRIPT_DIR + '/generate_binary_info_' + datetime.now().isoformat()
...
handle_folder(starting_dir, bench_json['flags'], bench_json['benchmarks'], dir)
